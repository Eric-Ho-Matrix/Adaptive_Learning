...
def compare_potenial_and_threshold(potential, threshold):
    """
    Compares the membrane potential of a neuron with a threshold value.
    ...
    threshold : float
        The threshold value for spiking.

    Returns:
    --------
    bool
        True if the potential exceeds the threshold, indicating a spike.
    """
    ...
    for b in range(nb_blocks):  # nb_blocks
        ...
        for t in range(total_trials):  # nb_trials
            ...
            while 1:
                ...
                switcher_marker = compare_potenial_and_threshold(spike_neuron.membrane_potential, spike_neuron.threshold)
                if switcher_marker == True:
                    bias_inh = BASE_TH
...
plt.title('LEIA Changepoint Model with adaptive threshold', fontsize=20)
