"""
Calculate the best entropy threshold for each participant
and calculate the LR against the trial number after the changepoints
"""
...
for gain_on_mem in GAIN:
    for j_inh in range(num_of_inh_th):
        ### need to read data from 30 dataset
        dir = "grid_"+str(gain_on_mem)+"_bias_inh_"+str(round(float(j_inh*0.2+1),1))
        npz_file = dir+"/Model_simulations_for_LR_dynamics_grid_"+str(gain_on_mem)+"_bias_"+str(round(float(j_inh*0.2+1),1))+".npz"
        ...
        for i in range(nSim):    ## participant 17 and 18 adjustment    17 blk1 and 18 blk3 only have 119 trials    so we use 119 trials of blk1 and blk3 across each ppant
            ...
            mse = mean_squared_error(model_pred,pred)
            ...
            Human_lr = modelSubCE_H.reshape(-1,1)
            Model_lr = modelSubCE.reshape(-1,1)

            MSE[j_inh, i] = mean_squared_error(Human_lr, Model_lr)


        REC_BETAS.append(rec_betas)


#### get the best Entropy Threshold for each ppants 
fig, ax = plt.subplots(1, 1, figsize=(10, 5))
im = ax.imshow(MSE, extent=[1,32,7,1])
ax.set_title('Entropy Threshold Search for Each Participant', fontsize=15)
divider = make_axes_locatable(ax)
cax = divider.append_axes("right", size="5%", pad=0.05)
clb = fig.colorbar(im, cax=cax)
clb.set_label('MSE Values', labelpad=17,fontsize=15, rotation=270)
ax.set_xlabel('Participant', fontsize=15)
ax.set_ylabel("Entropy Threshold",labelpad=10, fontsize=15)
plt.tight_layout()
# plt.savefig('MSE_BY_LR.svg', format='svg',dpi=500, bbox_inches='tight')
plt.show()

# use best cosine similarity
Best_threshold = []
for i in range(32):
    index = np.argmin(MSE[:,i])
    real_threshold = round((index*0.2 + 1), 1)
    Best_threshold.append(real_threshold)

print(Best_threshold)

# save model
# here we save the best threshold for each ppant
np.savez("best_threshold_for_mse_lr", Best_threshold)

# draw the scatter
xx = [i for i in range(32)]
plt.figure()
plt.scatter(xx, Best_threshold, label='Best Entropy Threshold')
plt.title('Best Entropy Threshold for Each Participant')
plt.xlabel('Participants')
plt.ylabel('Entropy Threshold')
plt.legend()
# plt.savefig('Best_threshold.svg', format='svg',dpi=500, bbox_inches='tight')
plt.show()
#### End calculate Entropy 


#### get the best Entropy Threshold on average
human_lr_avg = np.mean(rec_betas_H,axis=0).reshape(-1,1)
MSE = []
for i in range(len(REC_BETAS)):
    model_lr = np.mean(REC_BETAS[i], axis = 0).reshape(-1,1)
    MSE.append(mean_squared_error(human_lr_avg, model_lr))

index = np.argmin(MSE)
best_entro_th_avg = round(index*0.2 + 1,1)
print("Best entropy Threshold on Average:", best_entro_th_avg)

# draw the plot
error_H = np.std(rec_betas_H, axis=0)
error = np.std(REC_BETAS[index], axis=0)

plot_list = [i+1 for i in range(trial_number)]

xx = np.arange(0,trial_number,1)
plt.figure(figsize=(7,5))
plt.plot(xx,np.mean(REC_BETAS[index],axis=0), '--', linewidth = 5 , color = 'purple', label = 'LEIA Model' )
plt.fill_between(xx, np.mean(REC_BETAS[index],axis=0)-error, np.mean(REC_BETAS[index],axis=0)+error, alpha=0.5, facecolor = 'purple')
plt.plot(xx,np.mean(rec_betas_H,axis=0), '--', linewidth = 5 , color = 'blue', label = 'Human' )
...
# plt.savefig('Best_lr.svg', format='svg',dpi=300, bbox_inches='tight')
plt.show()
# end draw
